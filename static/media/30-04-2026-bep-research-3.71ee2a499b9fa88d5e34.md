...
The encoding process and the difficulty in disentangling the representation space (making the concepts/features separate from each other), faces few critical problems. In the ideal scenario, when a concept is present in an image we would have a single corresponding neuron which activates for that image - there exists a bijective function between the concept space and the neurons.


Firstly, what is a concept? or a feature? I use these two terms synonymously, and Neel Nanda defines "A **feature** is a property of an input to the model, or some subset of that input (eg a token in the prompt given to a language model, or a patch of an image)" [ref](https://www.neelnanda.io/mechanistic-interpretability/glossary) Since here we are focused on visual models, features can range from simple ones like lines, curves or colors, to higher in complexity objects like wheels or frames, as well as objects comprising multiple lower level objects like cars. Evidence for models encoding those at different layers have been extensively shown [e.g. ref](https://arxiv.org/pdf/1311.2901), so we assume that these do in fact exist and focus on how they are encoded and what are the difficulties.
...
[//]: # ()
[//]: # (### Does the LLM interpretability research apply in visual domain?)
