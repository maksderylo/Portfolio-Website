...
This looks like U-Net with the activation space being the bottleneck layers. We cannot just utilize that architecture, because we loose the per neuron interpretability (mixing of the channels). We implement a grouped convolutional decoder:
```python
...
1. Understanding Deep Image Representations by Inverting Them [link](https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=7299155&tag=1)
"given an encoding of an image, to which extent is it possible to reconstruct the image itself" - they look into what data is lost (and what retained) when passing through an encoder. They are able to generate the images that are seen as important (without irrelevant parts) which could be linked to what my feature recon loss is meant to do.
Also they regularize by image priors to encourage natural-looking reconstructions, which I should learn to do.